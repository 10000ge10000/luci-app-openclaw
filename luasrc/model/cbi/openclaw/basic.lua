...
act.cfgvalue = function(self, section)
	...
	html[#html+1] = '<div><strong style="color:#333;">🆕 最新版（未经插件测试，可能不兼容）</strong>'
	html[#html+1] = '<div style="font-size:12px;color:#e36209;margin-top:4px;">⚠️ 安装 npm 上的最新发布版本，可能存在未经验证的兼容性问题。</div>'
		html[#html+1] = '</div></label>'
		html[#html+1] = '<label style="display:flex;align-items:flex-start;gap:10px;padding:14px 16px;border:2px solid #e0e0e0;border-radius:8px;cursor:pointer;background:#fff;" id="oc-opt-extended-stable">'
		html[#html+1] = '<input type="radio" name="oc-ver-choice" value="extended-stable" style="margin-top:2px;">'
		html[#html+1] = '<div><strong style="color:#333;">🛡️ 扩展稳定版</strong><div style="font-size:12px;color:#666;margin-top:4px;">npm extended-stable 标签，通常比 latest 更保守，但仍未完成本插件全量验证。</div></div></label>'
		html[#html+1] = '</div>'
	-- 自定义安装路径
	...
	-- JavaScript
		...
		html[#html+1] = 'var csrfToken="<%=token%>";'
		html[#html+1] = 'function ocEscapeHtml(v){var d=document.createElement("div");d.textContent=String(v==null?"":v);return d.innerHTML;}'
		html[#html+1] = 'function ocSetText(el,text,color){el.textContent=String(text==null?"":text);if(color)el.style.color=color;}'
	...
	html[#html+1] = 'if(!path){path="/opt";pathEl.value=path;}'
		html[#html+1] = 'ocSetText(infoEl,"⏳ 正在检测空间...");'
	...
	html[#html+1] = 'if(r.disk_ok){'
		html[#html+1] = 'ocSetText(infoEl,"✅ 可用空间: "+r.disk_free_str+" (检测路径: "+r.disk_path+")","#1a7f37");'
	html[#html+1] = '}else{'
		html[#html+1] = 'ocSetText(infoEl,"❌ 空间不足: "+r.disk_mb+" MB 可用，需要 ≥ 2048 MB (检测路径: "+r.disk_path+")","#cf222e");'
	html[#html+1] = '}'
		html[#html+1] = '}catch(e){ocSetText(infoEl,"⚠️ 检测失败","#e36209");}'
	html[#html+1] = '});'
	html[#html+1] = '}'
	...
	html[#html+1] = 'for(var i=0;i<radios.length;i++){if(radios[i].checked){choice=radios[i].value;break;}}'
		html[#html+1] = 'var verParam=choice;'
	...
	html[#html+1] = 'logEl.textContent+="════════════════════════════════════════\\n";'
	html[#html+1] = 'logEl.textContent+="📦 安装运行环境 ("+(version==="stable"?"稳定版":(version==="extended-stable"?"扩展稳定版":"最新版（未经插件测试）"))+")\\n";'
	html[#html+1] = 'logEl.textContent+="════════════════════════════════════════\\n";'
	html[#html+1] = 'logEl.textContent+="安装路径: "+install_path+"\\n";'
	html[#html+1] = 'logEl.textContent+="正在启动安装...\\n";'
		html[#html+1] = '(new XHR()).post("' .. ctl_url .. '",{token:csrfToken,action:"setup",version:version,install_path:install_path},function(x){'
	...
	html[#html+1] = '"<div style=\\"margin-top:8px;font-size:12px;color:#666;\\">💡 完整日志见上方终端输出，也可在终端查看：<code>cat /var/run/openclaw/setup.log</code></div></div>";'
	html[#html+1] = '}'
	html[#html+1] = '}'
	...
	html[#html+1] = 'var el=document.getElementById("action-result");'
		html[#html+1] = 'ocSetText(el,"⏳ 正在执行...","#999");'
		html[#html+1] = '(new XHR()).post("' .. ctl_url .. '",{token:csrfToken,action:action},function(x){'
	html[#html+1] = 'try{var r=JSON.parse(x.responseText);'
		html[#html+1] = 'if(r.status==="ok"){ocSetText(el,"✅ "+action+" 已完成","green");}'
		html[#html+1] = 'else{ocSetText(el,"❌ "+(r.message||"失败"),"red");}'
		html[#html+1] = '}catch(e){ocSetText(el,"❌ 错误","red");}'
	html[#html+1] = '});}'
	...
	-- 链接 [text](url) - 悬停效果
		html[#html+1] = 'html=html.replace(/\\[([^\\]]+)\\]\\(([^)]+)\\)/g,function(m,label,url){var safe=/^(?:https?:|mailto:|#)/i.test(url)?url:"#";return"<a href=\\""+ocEscapeHtml(safe)+"\\" target=\\"_blank\\" rel=\\"noopener\\" style=\\"color:#0969da;text-decoration:none;border-bottom:1px solid transparent;transition:border-color 0.2s;\\">"+label+"</a>";});'
	-- 无序列表 (- 或 *)
	...
	html[#html+1] = 'if(r.plugin_current){'
		html[#html+1] = 'if(r.plugin_has_update){msgs.push("<span style=\\""+badgeNew+"\\">🔌 有新版本</span> v"+ocEscapeHtml(r.plugin_current)+" → <span style=\\""+verBadge+"\\">v"+ocEscapeHtml(r.plugin_latest)+"</span>");}'
		html[#html+1] = 'else if(r.plugin_latest){msgs.push("<span style=\\""+badgeLatest+"\\">✅ 已是最新</span> v"+ocEscapeHtml(r.plugin_current));}'
		html[#html+1] = 'else{msgs.push("<span style=\\""+badgeUnknown+"\\">🔌 无法检查</span> v"+ocEscapeHtml(r.plugin_current));}'
	html[#html+1] = '}'
	...
	html[#html+1] = 'var ver=window._pluginLatestVer;'
		html[#html+1] = 'if(!ver||!/^[0-9A-Za-z._-]+$/.test(String(ver))){alert("无法获取有效的最新版本号");return;}'
	...
	html[#html+1] = 'resultEl.style.display="none";'
		html[#html+1] = '(new XHR()).post("' .. plugin_upgrade_url .. '",{token:csrfToken,version:ver},function(x){'
	html[#html+1] = 'try{JSON.parse(x.responseText);}catch(e){}'
	...
	html[#html+1] = '"<span style=\\"color:#555;font-size:13px;\\">请查看上方日志了解详情。也可手动执行：<code>cat /var/run/openclaw/plugin-upgrade.log</code></span><br/>"+'
	html[#html+1] = '"<button class=\\"btn cbi-button cbi-button-apply\\" type=\\"button\\" onclick=\\"location.reload()\\" style=\\"margin-top:10px;\\">🔄 刷新页面</button></div>";'
	html[#html+1] = '}'
	html[#html+1] = '}'
	...
	html[#html+1] = 'el.innerHTML="<span style=\\"color:#999\\">正在停止服务并清理文件...</span>";'
		html[#html+1] = '(new XHR()).post("' .. uninstall_url .. '",{token:csrfToken},function(x){'
	...
	html[#html+1] = '"<span style=\\"color:#555;font-size:13px;\\">"+ocEscapeHtml(r.message)+"</span><br/>"+'
	...
	html[#html+1] = '}else{el.innerHTML="<span style=\\"color:red\\">❌ "+ocEscapeHtml(r.message||"卸载失败")+"</span>";}'
	...
	html[#html+1] = 'el.innerHTML="<div style=\\"color:#7aa2f7;font-size:12px;padding:8px;\\">⏳ 加载备份列表...</div>";'
		html[#html+1] = '(new XHR()).post("' .. backup_url .. '",{token:csrfToken,action:"list"},function(x){'
	html[#html+1] = 'try{var r=JSON.parse(x.responseText);'
	...
	html[#html+1] = 'var safeFilename=String(b.filename||"").replace(/[^A-Za-z0-9._-]/g,"");'
	...
	html[#html+1] = '"<td style=\\"padding:7px 8px;color:#555;white-space:nowrap;\\">"+ocEscapeHtml(b.time)+"</td>"+' -- 时间
	html[#html+1] = '"<td style=\\"padding:7px 8px;text-align:right;color:#666;white-space:nowrap;\\">"+ocEscapeHtml(b.size_str)+"</td>"+' -- 大小
	html[#html+1] = '"<td style=\\"padding:5px 8px;text-align:center;white-space:nowrap;\\">"+'
	html[#html+1] = '"<button class=\\"btn cbi-button cbi-button-action\\" style=\\"font-size:11px;padding:1px 8px;margin-right:4px;\\" data-file=\\""+ocEscapeHtml(safeFilename)+"\\" onclick=\\"ocRestoreBackup(this.dataset.file)\\">恢复</button>"+'
	html[#html+1] = '"<button class=\\"btn cbi-button cbi-button-remove\\" style=\\"font-size:11px;padding:1px 8px;\\" data-file=\\""+ocEscapeHtml(safeFilename)+"\\" onclick=\\"ocDeleteBackup(this.dataset.file)\\">删除</button>"+'
	html[#html+1] = '"</td></tr>";'
	html[#html+1] = '}'
	html[#html+1] = 'h+="</table>";'
	...
	html[#html+1] = '}else{'
	html[#html+1] = 'el.innerHTML="<div style=\\"color:#e36209;font-size:12px;padding:8px;\\">⚠️ "+ocEscapeHtml(r.message||"获取列表失败")+"</div>";'
	html[#html+1] = '}'
	...
	html[#html+1] = 'resEl.innerHTML="<div style=\\"color:#7aa2f7;font-size:12px;padding:8px;\\">⏳ 正在创建备份..."+(onlyConfig?"（仅配置）":"（完整备份，可能需要较长时间）")+"</div>";'
		html[#html+1] = '(new XHR()).post("' .. backup_url .. '",{token:csrfToken,action:"create",only_config:onlyConfig},function(x){'
	...
	html[#html+1] = 'ocLoadBackupList();'
	html[#html+1] = '}else{'
	html[#html+1] = 'resEl.innerHTML="<div style=\\"color:#e36209;font-size:12px;padding:8px;\\">⚠️ "+ocEscapeHtml(r.message||"备份失败")+"</div>";'
	html[#html+1] = '}'
	...
	html[#html+1] = 'resEl.innerHTML="<div style=\\"color:#7aa2f7;font-size:12px;padding:8px;\\">⏳ 正在恢复配置...</div>";'
		html[#html+1] = '(new XHR()).post("' .. backup_url .. '",{token:csrfToken,action:"restore",file:filename},function(x){'
	html[#html+1] = 'try{var r=JSON.parse(x.responseText);'
	html[#html+1] = 'if(r.status==="ok"){'
	html[#html+1] = 'resEl.innerHTML="<div style=\\"border:1px solid #c6e9c9;background:#e6f7e9;padding:10px 14px;border-radius:6px;font-size:12px;\\">"+'
	html[#html+1] = '"<strong style=\\"color:#1a7f37;\\">✅ 配置已恢复</strong><br/>"+'
	html[#html+1] = '"<span style=\\"color:#555;\\">"+ocEscapeHtml(r.message)+"</span><br/>"+'
	...
	html[#html+1] = '}else{'
	html[#html+1] = 'resEl.innerHTML="<div style=\\"color:#cf222e;font-size:12px;padding:8px;\\">❌ "+ocEscapeHtml(r.message||"恢复失败")+"</div>";'
	html[#html+1] = '}'
	...
	html[#html+1] = 'resEl.innerHTML="<div style=\\"color:#7aa2f7;font-size:12px;padding:8px;\\">⏳ 正在删除...</div>";'
		html[#html+1] = '(new XHR()).post("' .. backup_url .. '",{token:csrfToken,action:"delete",file:filename},function(x){'
	html[#html+1] = 'try{var r=JSON.parse(x.responseText);'
	html[#html+1] = 'if(r.status==="ok"){'
	html[#html+1] = 'resEl.innerHTML="<div style=\\"border:1px solid #c6e9c9;background:#e6f7e9;padding:10px 14px;border-radius:6px;font-size:12px;\\">"+'
	html[#html+1] = '"<strong style=\\"color:#1a7f37;\\">✅ "+ocEscapeHtml(r.message)+"</strong></div>";'
	html[#html+1] = 'ocLoadBackupList();'
	html[#html+1] = '}else{'
	html[#html+1] = 'resEl.innerHTML="<div style=\\"color:#cf222e;font-size:12px;padding:8px;\\">❌ "+ocEscapeHtml(r.message||"删除失败")+"</div>";'
	html[#html+1] = '}'
